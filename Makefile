...
SPEC_LEVS = asmodeus.lev baalz.lev bigrm-*.lev castle-?.lev fakewiz?.lev \
    ...
    heck-*.lev unhck-*.lev exmall-?.lev hellma-?.lev \
    sheolfil.lev sheolmid.lev palace_e.lev palace_f.lev \
...
# set up some additional files
	touch $(FILE_AREA_VAR)/perm $(FILE_AREA_VAR)/record $(FILE_AREA_VAR)/logfile $(FILE_AREA_VAR)/xlogfile
	-( cd $(FILE_AREA_VAR) ; $(CHOWN) $(GAMEUID) perm record logfile xlogfile ; \
